...
class CustomResNet(torch.nn.Module): 
    def __init__(self, num_classes, class_out): # 256, 3
        super().__init__()
        net = resnet34(spatial_dims=2, n_input_channels=4, num_classes=num_classes) #----------- changed n_input_channels=1
        # self.dropout = 
    ...
    def forward(self, x, labels, add_vars):
        ...
        loss2 = self.mse_agewtht(x2, add_vars[:,1:4].reshape(x2.shape).to(torch.float32)) 
...
class MbdDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        return self.transforms(self.image_files1[index]), \
            self.transforms(self.image_files2[index]), \
            self.transforms(self.image_files3[index]), \
            self.transforms(self.image_files4[index]), \
            self.labels[index], self.params[index,:] # self.transforms(self.image_files2[index]), self.transforms(self.image_files3[index]), self.transforms(self.image_files4[index]), self.labels[index], self.params[index,:]
...
for fold, (train_indices_tmp,val_indices_tmp) in enumerate(splits.split(indices[test_split:])):
    ...
    for epoch in range(max_epochs):
        ...
        if (epoch + 1) % val_interval == 0:
            ...
            with torch.no_grad():
                ...
                if result > best_metric: # result for val
                    best_metric = result
    ...
    print(f"fold{fold} - train completed, best F1: {best_metric:.4f} " f" at epoch: {best_metric_epoch}")

pkl.dump(indice_dict, open(f'../data/logs/indices_{trial_name}.pkl','wb'))

# python /home/chenfy/projects/classifier2/codes/train_1A5.py > /home/chenfy/projects/classifier2/logs/072223_1A5.log